...
def run_nsubj_dim(run, B = 100, simtype = 1, niters = 5000 ):
    ...
    for J in np.arange(len(dim_sides)):
        ...
        for I in np.arange(len(nsubj_vec)):
            print('I:', I)
            FWER_FPR, JER_FPR = pr.bootfpr(Dim, nsubj_vec[I], C, fwhm, 0, B, niters, pi0, simtype)
...
def run_nsubj_dim_mean(run, B = 100, simtype = 1, niters = 5000):
    ...
    saveloc += 'meanpropdim2D' + '_fwhm_' + str(fwhm) + '_pi0_' + str(int(100*pi0))
    saveloc += '_B_' + str(B) + '_niters_' + str(niters) + '_simtype_' + str(simtype)
...
def run_nsubj_dim_update(run, B = 100, simtype = 1, niters = 5000 ):
    ...
    saveloc += 'nsubjdim2D_fwhm_' + str(fwhm) + '_pi0_' + str(int(100*pi0)) + '_nsubj_' + str(nsubj)
    saveloc += '_B_' + str(B) + '_niters_' + str(niters) + '_simtype_' + str(simtype)
    ...
    for J in np.arange(len(dim_sides)):
        ...
        FWER_FPR, JER_FPR, FWER_FPR_SD, JER_FPR_SD = pr.bootfpr(Dim, nsubj, C, fwhm, 0, B, niters, pi0, simtype, do_sd = 1)
        store_JER[0,J] = JER_FPR
        store_FWER[0,J] = FWER_FPR
        store_JER_SD[0,J] = JER_FPR_SD
        store_FWER_SD[0,J] = FWER_FPR_SD
        np.savez(saveloc + '.npz', JER_FPR  = store_JER, FWER_FPR = store_FWER, JER_FPR_SD = store_JER_SD, FWER_FPR_SD = store_FWER_SD)
