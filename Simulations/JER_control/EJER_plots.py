...
parametric_results_dir = 'C:\\Users\\12SDa\\davenpor\\davenpor\\Toolboxes\\Papers\\lmfdp\\Simulations\\JER_control\\EJER_runs_data\\Parametric\\'
bootstrap_results_dir = 'C:\\Users\\12SDa\\davenpor\\davenpor\\Toolboxes\\Papers\\lmfdp\\Simulations\\JER_control\\EJER_runs_data\\Bootstrap\\'

nsubj = 50;
load_results = np.load(parametric_results_dir + 'nsubjdim2D_fwhm_0_pi0_100_nsubj_' + str(nsubj) + '_B_100_niters_5000_simtype_-1.npz')
...
load_results = np.load(bootstrap_results_dir + 'nsubjdim2D_fwhm_0_pi0_' + str_pi0 + '_nsubj_' + str(nsubj) + '_B_100_niters_5000_simtype_1.npz')
JER_FPR = load_results['JER_FPR']
print(JER_FPR)
    ...
    for j in np.arange(len(dim_sides)):
        ...
        for i in np.arange(len(FWHM_vec)):
            ...
            for k in np.arange(len(nsubj_vec)):
                ...
                parametric_results = np.load(parametric_results_dir + 'nsubjdim2D_fwhm_' + str(FWHM_vec[i]) + '_pi0_' + str(pi0invec) + '_nsubj_' + str(nsubj_vec[k]) + '_B_100_niters_5000_simtype_-1.npz')
                
                # Load the bootstrap results
                bootstrap_results = np.load(bootstrap_results_dir + 'nsubjdim2D_fwhm_' + str(FWHM_vec[i]) + '_pi0_' + str(pi0invec) + '_nsubj_' + str(nsubj_vec[k]) + '_B_100_niters_5000_simtype_1.npz')
            ...
            plt.plot(nsubj_vec, observed_jer_simes[0], label = 'Simes', color = paracolor,linestyle = 'dashed', linewidth = lw)
            plt.plot(nsubj_vec, observed_jer_ari[0], label = 'ARI', color = paracolor, linewidth = lw)
            plt.plot(nsubj_vec, observed_jer_boot[0], label = 'Bootstrap', color = bootcolor, linestyle = 'dashed', linewidth = lw)
            plt.plot(nsubj_vec, observed_jer_boot_sd[0], label = 'Bootstrap Step Down', color = bootcolor, linewidth = lw)
            ...
            plt.title("FWHM = " + str(FWHM_vec[i]) + " and $\pi_0$ = " + str(pi0))
            ...
            saveloc = 'C:\\Users\\12SDa\\davenpor\\davenpor\\Toolboxes\\Papers\\lmfdp\\Simulations\\JER_control\\EJER_plots\\'
            ...
            plt.savefig(saveloc + 'JER_dim_' + str(dim) + '_fwhm_' + str(FWHM_vec[i]) + '_pi0_' + str(pi0invec) + str(biomaddon) + '.pdf')
            plt.close()
            ...
            plt.plot(nsubj_vec, observed_fwer_simes[0], label = 'Simes', color = paracolor,linestyle = 'dashed', linewidth = lw)
            plt.plot(nsubj_vec, observed_fwer_ari[0], label = 'Holm', color = paracolor, linewidth = lw)
            plt.plot(nsubj_vec, observed_fwer_boot[0], label = 'Bootstrap', color = bootcolor, linestyle = 'dashed', linewidth = lw)
            plt.plot(nsubj_vec, observed_fwer_boot_sd[0], label = 'Bootstrap Step Down', color = bootcolor, linewidth = lw)
            ...
            plt.title("FWHM = " + str(FWHM_vec[i]) + " and $\pi_0$ = " + str(pi0))
            #saveloc = 'C:\\Users\\12SDa\\global\\Intern\\drago\\Figures\\FinalEJerPlots\\'
            saveloc = 'C:\\Users\\12SDa\\davenpor\\davenpor\\Toolboxes\\Papers\\lmfdp\\Simulations\\JER_control\\FWER_plots\\'
            plt.subplots_adjust(left=0.17, right=0.95, bottom=0.17, top=0.9)
            plt.savefig(saveloc + 'FWER_dim_' + str(dim) + '_fwhm_' + str(FWHM_vec[i]) + '_pi0_' + str(pi0invec) + str(biomaddon) + '.pdf')
            plt.close()