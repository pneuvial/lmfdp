...
def run_power_sims(run, nbootstraps = 100, simtype = 1, niters = 5000, FWHM = 4):
    ...
    runmod5 = run % 5
    nsubj = (runmod5+1)*10
    power, power_sd = pr.bootpower(dim, nsubj, C, FWHM, 0, nbootstraps, niters, pi0, simtype)
    if simtype > -1:
         np.savez(saveloc + 'nsubj_' + str(nsubj) + '_pi0_' + str(int(100*pi0)) + '_nbootstraps_' + str(nbootstraps) + '_niters_' + str(niters) + '_dim_' + str(dimside) + str(dimside) + '_simtype_' + str(simtype), power = power, power_sd = power_sd)
    else:
         np.savez(saveloc + 'nsubj_' + str(nsubj) + '_pi0_' + str(int(100*pi0)) + '_niters_' + str(niters) + '_dim_' + str(dimside) + str(dimside) + '_simtype_' + str(simtype), power = power, power_sd = power_sd)
...
def run_power_boot_sims(nbootstraps, niters = 5000, FWHM = 4):
    ...
    # Calculate and save the power
    power, power_sd = pr.bootpower(dim, nsubj, C, FWHM, 0, nbootstraps, niters, pi0, 1)
    np.savez(saveloc + 'nsubj_' + str(nsubj) + '_pi0_' + str(int(100*pi0)) + '_nbootstraps_' + \
             str(nbootstraps) + '_niters_' + str(niters) + '_dim_' + str(dimside) + str(dimside), \
...
def run_power_sims_unrunsims(run, nbootstraps = 100, niters = 5000, FWHM = 4):
    ...
    power = pr.bootpower(dim, nsubj, C, FWHM, 0, nbootstraps, niters, pi0, simtype)
    np.savez(saveloc + 'nsubj_' + str(nsubj) + '_pi0_' + str(int(100*pi0)) + '_nbootstraps_' + str(nbootstraps) + '_niters_' + str(niters) + '_dim_' + str(dimside) + str(dimside) + '_simtype_' + str(simtype), power = power)
