...
restricted_vars2use = ['Age_in_Yrs', 'Height', 'Weight', 'BMI', 'BPSystolic', 'BPDiastolic', 'Handedness']
...
# Use 100 subjects for now and restrict the number of images
nsubj = len(imgs)
imgs = imgs[0:nsubj]

# Obtain the design and contrast matrices
design, contrast_matrix = lm_setup(restricted_vars2use, unrestricted_vars2use, contrast_vars, df_unrelated, df_ur_unrelated, nsubj)
...
for simtype in np.array((-1,1)):
    tdp_bounds, tdp_bounds_sd, lambda_quant, lambda_quant_sd, masker = pr.cluster_tdp_brain(imgs, design, contrast_matrix, mask, n_bootstraps = n_bootstraps, min_cluster_size = 1, simtype = simtype)
    ...
    saveloc = '/storage/store2/work/sdavenpo/drago/LinearModelFitting/tdp_bounds_' + pair + '_' + method

    if method == 'boot':
        saveloc = saveloc + '_B_'+ str(n_bootstraps) 
    
    np.savez(saveloc + '.npz', tdp_bounds = tdp_bounds, tdp_bounds_sd = tdp_bounds_sd, lambda_quant = lambda_quant, lambda_quant_sd = lambda_quant_sd)
    ...
    for L in np.arange(n_contrasts):
        ...
        if method == 'boot':
            plt.savefig(image_dir + contrast_vars[L] + '_TDP_' + pair + '_' + method + '_B_'+ str(n_bootstraps) + '.pdf')
        else:
            plt.savefig(image_dir + contrast_vars[L] + '_TDP_' + pair + '_' + method + '.pdf')
