...
for method in ['parametric', 'boot']:
    ...
    # Obtain the masker
    fwhm = 4 # the fwhm that was used to generate things (shouldn't affect things here I don't think)
...
c_size_index = np.argsort(-c_sizes_including_sex) # Get the sort indices in decreasing order
...
for greyscale in np.arange(2):
    ...
    plt.plot(np.arange(nclusters), ari_cluster_tdps[c_size_index], '*', color = paracolor, label = 'ARI')
    plt.plot(np.arange(nclusters), simes_cluster_tdps[c_size_index], '+', color = paracolor, label = 'Simes')
    plt.plot(np.arange(nclusters), boot_cluster_tdps[c_size_index], '*', color = bootcolor, label = 'bootstrap')
    ...
    plt.plot(np.arange(nclusters), ari_cluster_tps[c_size_index], '*', color = paracolor, label = 'ARI')
    plt.plot(np.arange(nclusters), simes_cluster_tps[c_size_index], '+', color = paracolor, label = 'Simes')
    plt.plot(np.arange(nclusters), boot_cluster_tps[c_size_index], '*', color = bootcolor, label = 'bootstrap')
...
bootsgreaterthansimes = 1*(tdp_bounds_boot[..., index] > 0) - (tdp_bounds_simes[..., index] > 0)
boot_extra_clusters = tdp_bounds_boot[np.array(bootsgreaterthansimes, dtype = 'bool'), index]
